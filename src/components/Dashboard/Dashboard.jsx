...
const navigation = [
  ...
  [Route, "Particle Backtracking", "/dashboard/particle-backtracking"],
  [ShipWheel, "Source Attribution", "/dashboard/source-attribution"],
  ...
  [ShieldCheck, "User Management", "/dashboard/user-management"],
];
const kpis = [
...
const searchResults = [
  ...
  ["Particle Backtracking", "Probable source hindcast", "/dashboard/particle-backtracking"],
  ["Source Attribution", "AIS candidate ranking", "/dashboard/source-attribution"],
...
export function Dashboard({ children }) {
  ...
  const suggestions = query.trim()
    ? searchResults
        .filter(([title, detail]) =>
          `${title} ${detail}`.toLowerCase().includes(query.toLowerCase()),
        )
        .slice(0, 6)
    : [];
    ...
    <div className={`dashboard-page${collapsed ? " sidebar-collapsed" : ""}`}>
      <aside className="dashboard-sidebar">
        ...
        <div className="watch-status">
          ...
          <span>Next SAR pass 09:42 UTC · Sentinel-1A</span>
        </div>
      ...
      <main className="dashboard-content">
        <header className="dashboard-topbar">
          ...
          <div className="dashboard-search-wrap">
            <div className="dashboard-search">
              ...
              <input
                ...
                placeholder="Search incidents, vessels, zones, coordinates..."
              />
            </div>
            {suggestions.length > 0 && (
              <div className="search-suggestions">
                {suggestions.map(([title, detail, path]) => (
                  <button
                    onClick={() => openSearchResult(path)}
                    key={`${title}-${path}`}
                  >
                    <Search size={14} />
                    <span>
          ...
          <div className="dashboard-date">
            <CalendarDays size={16} /> 25 Aug 2026 · 06:58 UTC
          </div>
          ...
          {children ?? (
            <>
              ...
              >
                ...
                <div className="heading-actions">
                  ...
                  <button
                    ...
                    onClick={() => navigate("/dashboard/ai-simulation")}
                  >
              ...
              <div className="dash-kpi-grid">
                {kpis.map((item, index) => (
                  <KpiCard item={item} index={index} key={item[0]} />
                ))}
              </div>
              <div className="dashboard-grid">
                <section className="dashboard-card map-card">
                  <div className="card-header">
                    <div>
                      <h2>Maritime monitoring — Gulf of Khambhat</h2>
                      <p>
                        Sentinel-1A 06:02 UTC · slick extent, forecast track and
                        response assets
                      </p>
                    </div>
                  ...
                  <div className="command-map">
                    ...
                    <div className="slick-blob" />
                    <div className="forecast-line" />
                    <i className="map-marker marker-one" />
                    <i className="map-marker marker-two" />
                    <i className="map-marker marker-three" />
                    <span className="asset-label label-one">
                      GEOSENCE GF-02<small>Exclusion · 4.1 km²</small>
                    </span>
                  ...
                  <div className="map-footer">
                    <span>
                      <i className="legend-slick" /> Slick extent
                    </span>
                    <span>
                      <i className="legend-forecast" /> Forecast track
                    </span>
                    <span>
                      <i className="legend-assets" /> Response assets
                    </span>
                    <b>
                      <Clock3 size={14} /> Updated 06:22 UTC
                    </b>
                  </div>
                </section>
                <section className="dashboard-card risk-card">
                  <div className="card-header">
                    <div>
                      ...
                      <p>Ensemble v4.2 · 12 members · updated 06:22 UTC</p>
                    </div>
                    <ShieldCheck className="risk-icon" size={22} />
                  </div>
                  <div className="risk-list">
                    <RiskBar
                      label="Shoreline contact probability"
                      place="Hazira / Suvali stretch"
                      value={78}
                      color="red"
                    />
                    <RiskBar
                      label="Ecological sensitivity exposure"
                      place="Mangrove priority zone"
                      value={64}
                      color="amber"
                    />
                    <RiskBar
                      label="Response window remaining"
                      place="Recommended action window"
                      value={42}
                      color="blue"
                    />
                  </div>
                ...
                <section className="dashboard-card incidents-card">
                  <div className="card-header">
                    <div>
                      ...
                      <p>Latest changes across your operational watch</p>
                    </div>
                    <button
                      className="card-link"
                  ...
                  {incidents.map(([id, place, time, severity, color]) => (
                    <button
                ...
                <section className="dashboard-card analytics-card">
                  <div className="card-header">
                    <div>
                      ...
                      <p>Monitoring footprint · last 7 days</p>
                    </div>
                    <button
                  ...
                  >
                    <path d="M0 93 C45 84 52 62 94 71 S145 82 180 54 S235 77 275 45 S330 54 362 26 S420 44 454 30 S510 42 560 10" />
                    <path
                      className="chart-area"
                      d="M0 93 C45 84 52 62 94 71 S145 82 180 54 S235 77 275 45 S330 54 362 26 S420 44 454 30 S510 42 560 10 V120 H0Z"
                    />
                  </svg>
                </section>
              </div>
...
function RiskBar({ label, place, value, color }) {
  return (
    <div className="risk-row">
      ...
      <div className={`risk-track ${color}`}>
        <motion.span
          initial={{ width: 0 }}
          animate={{ width: `${value}%` }}
