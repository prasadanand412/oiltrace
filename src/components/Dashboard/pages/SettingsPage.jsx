  ...
  return (
    <Dashboard>
      ...
      <div className="settings-layout">
        <nav>
          {["Workspace", "Notifications", "Data connections", "Security"].map(
            (item) => (
              <button
        ...
        <div className="settings-panel">
          ...
          <p>Configure {tab.toLowerCase()} for your operational team.</p>
          {preferences.map((item) => (
            <button
              className="settings-toggle"
              onClick={() => setEnabled({ ...enabled, [item]: !enabled[item] })}
              key={item}
            >
              <span>
                {item}
